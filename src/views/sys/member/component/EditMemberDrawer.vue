...
el-drawer(:modelValue="show",:show-close="false" style="padding:20px" :title="props.id? '修改' : '新增'" @close="onClose(null)"  size="50%")
  el-form(style="margin-top:20px" label-width="150px" label-position="left")
    ...
    el-form-item(label="书籍押金",)
      el-input-number(v-model="form.deposit_price" placeholder="请输入优惠价")
...
onUpdated(async () => {
  if (show.value && props.id) {
    const {
      valid_days, original_price, special_price, borrow_books_limit, desc, main_title, once_borrow_books_limit, share_reward_cash_back_times, style, sub_title, summary, tiny_app_enable, deposit_price
    ...
    form.deposit_price = deposit_price / 100;
...
const onSave = async () => {
  ...
  if (props.id) {
    await editVip({
      biz_vip_id: props.id, valid_days, original_price: original_price * 100, special_price: special_price * 100, deposit_price: deposit_price * 100,
    ...
    await storeVip({ valid_days, original_price: original_price * 100, special_price: special_price * 100, deposit_price: deposit_price * 100, borrow_books_limit, desc, main_title, once_borrow_books_limit, share_reward_cash_back_times, style, sub_title, summary, tiny_app_enable, })
  }
  onClose(true);
};
</script>
