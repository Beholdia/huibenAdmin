		...
		<el-dialog :title="(acType === 'add' ? '新增' : '修改') + '菜单'" v-model="isShowDialog" width="769px"
				...
				<el-row :gutter="35">
					<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
						<el-form-item label="上级菜单">
							<el-cascader :options="menuData"
								:props="{ label: 'title', value: 'id', checkStrictly: true, emitPath: false }" placeholder="请选择上级菜单"
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="菜单名称" prop="menu_name">
							<el-input v-model="ruleForm.menu_name" placeholder="请填写菜单名称" clearable></el-input>
						</el-form-item>
					</el-col>
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
					...
					<template v-if="ruleForm.menu_type === '0' || ruleForm.menu_type === '1'">
						<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						...
						<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
							<el-form-item label="链接地址">
								<el-input v-model="ruleForm.link_url" placeholder="外链/内嵌时链接地址（http:xxx.com）" clearable
									:disabled="ruleForm.is_link === 0">
								</el-input>
							</el-form-item>
						</el-col>
					</template>
					...
					<template v-if="ruleForm.menu_type === '0' || ruleForm.menu_type === '1'">
						<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
							<el-form-item label="是否隐藏">
								<el-radio-group v-model="ruleForm.is_hide">
									<!-- <el-radio v-for="dict in visibleOptions" :key="dict.value" :label="dict.value">{{ dict.label
										}}</el-radio> -->
						...
						<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
							<el-form-item label="是否外链">
								<el-radio-group v-model="ruleForm.is_link" :disabled="ruleForm.is_iframe === 1">
						...
						<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
							<el-form-item label="是否内嵌">
								<el-radio-group v-model="ruleForm.is_iframe" @change="onSelectIframeChange">
			...
			<template #footer>
				<span class="dialog-footer">
					...
					<el-button type="primary" @click="onSubmit" size="default" :loading="loading">{{ acType === 'add' ? '新 增' :
			'修改' }}</el-button>
				</span>
			</template>
		</el-dialog>
	...
	setup(props, { emit }) {
		...
		const state = reactive({
			...
			ruleForm: {
				...
				isKeepAlive: 1, // 是否缓存
				is_affix: 0, // 是否固定
				link_url: '', // 外链/内嵌时链接地址（http:xxx.com），开启外链条件，`1、is_link:true 2、链接地址不为空`
				is_iframe: 0, // 是否内嵌，开启条件，`1、is_iframe:true 2、链接地址不为空`
			},
			// 表单校验
			rules: {
				parentId: [
					...
					if (props.acType === 'add') {
						...
						getMenuInfo(row.id).then(res => {
							...
							state.ruleForm = {
								id: data.id,
								pid: data.pid, // 上级菜单
								menu_type: '' + data.menu_type, // 菜单类型
								menu_name: data.title, // 菜单名称
								name: data.name, // 接口规则
								component: data.component, // 组件路径
								is_link: data.is_link, // 是否外链
								...
								is_hide: data.is_hide, // 是否隐藏
								isKeepAlive: data.is_cached, // 是否缓存
								is_affix: data.is_affix, // 是否固定
								link_url: data.link_url, // 外链/内嵌时链接地址（http:xxx.com），开启外链条件，`1、is_link:true 2、链接地址不为空`
								is_iframe: data.is_iframe, // 是否内嵌，开启条件，`1、is_iframe:true 2、链接地址不为空`
							}
						})
					}
				}
		...
		const onSelectIframeChange = () => {
			if (state.ruleForm.is_iframe === 1) state.ruleForm.is_link = 1;
		...
		const onSubmit = () => {
			...
			formWrap.validate((valid: boolean) => {
				if (valid) {
					...
					if (props.acType === 'add') {
						//添加
						addMenu({ ...state.ruleForm, menuName: state.ruleForm.menu_name }).then(() => {
						...
						//修改
						updateMenu({ ...state.ruleForm, menuName: state.ruleForm.menu_name }).then(() => {
